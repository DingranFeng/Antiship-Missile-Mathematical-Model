...
for theta=linspace(0.72,0.81,num)
    ...
    if sqrt((Pm(1,tC)-Pa(1,tC))^2+(Pm(2,tC)-Pa(2,tC))^2)>u*Lac/2
        con=con+1;
        error(con)=sqrt((Pm(1,tC)-Pa(1,tC))^2+(Pm(2,tC)-Pa(2,tC))^2)-u*Lac/2;
    end
end
figure
hist(error(1:con),1000);
xlabel('打击误差（m）');
ylabel('频次');
title('打击误差分布（未杀伤情况下）');
box on
grid on
display(['未杀伤情况下的平均打击误差为：',num2str(sum(error(1:con))/num),' m'])