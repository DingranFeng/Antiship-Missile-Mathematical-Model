...
rh=zeros(1,n);
ss=0;
for thetar=linspace(0.6532,1.3544,n) %选取第1问结果中命中率较高的发射角范围
    nh=0;
    ss=ss+1;
    for theta=linspace(thetar-range,thetar+range,num)
    Pm=zeros(3,N); %导弹位置
    ...
    if sqrt((Pm(1,tC)-Pa(1,tC))^2+(Pm(2,tC)-Pa(2,tC))^2)<=5.7*Lac/2
        nh=nh+1;
    end
    end
    rh(ss)=nh/num;
end
thetar=linspace(0.6532,1.3544,n);
plot(thetar,rh,'linewidth',1.2);
hold off
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
hold on
...
for thetar=linspace(0.6532,1.3544,n) %选取第1问结果中命中率较高的发射角范围
    ...
    if sqrt((Pm(1,tC)-Pa(1,tC))^2+(Pm(2,tC)-Pa(2,tC))^2)<=5.9*Lac/2
        nh=nh+1;
    end
    end
    rh(ss)=nh/num;
...
for thetar=linspace(0.6532,1.3544,n) %选取第1问结果中命中率较高的发射角范围
    ...
    if sqrt((Pm(1,tC)-Pa(1,tC))^2+(Pm(2,tC)-Pa(2,tC))^2)<=6.1*Lac/2
        nh=nh+1;
    end
    end
    rh(ss)=nh/num;
...
for thetar=linspace(0.6532,1.3544,n) %选取第1问结果中命中率较高的发射角范围
    ...
    if sqrt((Pm(1,tC)-Pa(1,tC))^2+(Pm(2,tC)-Pa(2,tC))^2)<=6.3*Lac/2
        nh=nh+1;
    end
    end
    rh(ss)=nh/num;
...
for thetar=linspace(0.6532,1.3544,n) %选取第1问结果中命中率较高的发射角范围
    ...
    if sqrt((Pm(1,tC)-Pa(1,tC))^2+(Pm(2,tC)-Pa(2,tC))^2)<=6.5*Lac/2
        nh=nh+1;
    end
    end
    rh(ss)=nh/num;
...
for thetar=linspace(0.6532,1.3544,n) %选取第1问结果中命中率较高的发射角范围
    ...
    if sqrt((Pm(1,tC)-Pa(1,tC))^2+(Pm(2,tC)-Pa(2,tC))^2)<=6.7*Lac/2
        nh=nh+1;
    end
    end
    rh(ss)=nh/num;
...
xlabel('发射角(rad)');ylabel('杀伤率');
title('发射角对杀伤率的影响  (u=导弹杀伤半径/航母半径)');
legend('u=5.7','u=5.9','u=6.1','u=6.3','u=6.5','u=6.7');
box on
grid on